...
export default function OrderItem({
  ...
  const handleIncrease = () => {
    if (maxQuantity !== undefined && safeQuantity >= maxQuantity) {
      showAlert(`Only ${maxQuantity} items available in stock.`, 'warning');
      return;
    }
    const newQty = safeQuantity + 1;
    setQuantity(newQty);
    ...
    <div className="group bg-white rounded-[24px] p-5 shadow-sm border border-gray-100 hover:shadow-md transition-all relative overflow-hidden mb-4">
      <div className="flex flex-col md:flex-row items-center gap-6">
        ...
        <div className="w-full md:w-32 h-32 rounded-2xl overflow-hidden bg-[#F4F5F7] flex-shrink-0 relative group-hover:scale-105 transition-transform duration-500">
          {image ? (
            <img src={image} alt={name} className="w-full h-full object-contain mix-blend-multiply p-2" />
          ) : (
        ...
        <div className="w-full md:w-40 flex justify-center">
          <div className="inline-flex items-center bg-gray-50 rounded-xl p-1 border border-gray-100">
            ...
            <button
              ...
              disabled={maxQuantity !== undefined && safeQuantity >= maxQuantity}
